...
if __name__ == '__main__':
    ...
    for epoch in range(start_epoch, max_iter):
        ...
        if epoch % val_iter == 0:
            ...
            with torch.no_grad():
                with tqdm(va_loader, file=tqdm_out, unit='batch') as tepoch:
                    for batch in tepoch:
                        ...
                        # Ensure dtype is long (int) for labels
                        seg = seg.long()
                        val_outputs = val_outputs.long()
                        val_labels = monai.networks.one_hot(seg, cfg.model.class_num)
                        val_outputs = monai.networks.one_hot(val_outputs, cfg.model.class_num)
            ...
            if metric.mean().item() > best_val_score:
                best_val_score = metric.mean().item()
