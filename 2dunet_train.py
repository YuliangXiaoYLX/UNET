...
LastEditTime: 2025-04-20 23:40:40
FilePath: /Downloads/UNET/2dunet_train.py
Description:
I Love IU
...
if __name__ == "__main__":
    ...
    post_pred = Compose([AsDiscrete(argmax=True, to_onehot=cfg.model.class_num, dim=1)])
    ...
    for epoch in range(start_epoch, max_iter):
        ...
        if epoch % val_iter == 0:
            ...
            valid = (not_nans.sum(dim=0) > 0)                   # [C-1]
            safe = torch.nan_to_num(class_dice, nan=0.0)
            per_class_epoch = safe.sum(dim=0) / not_nans.sum(dim=0).clamp(min=1)  # [C-1]
            per_class_epoch = torch.where(valid, per_class_epoch, torch.zeros_like(per_class_epoch))
            macro_mean = per_class_epoch[valid].mean().item() if valid.any() else 0.0
            worst_class = per_class_epoch[valid].min().item() if valid.any() else 0.0
